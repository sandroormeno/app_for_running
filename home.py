...
st.set_page_config(
    page_title="Route on bikeway in La Molina",
    page_icon="",
)

st.sidebar.success("Select a date above.")
...
def show_maps(view, samples, data):
    down = dicts[view][0]
    up = dicts[view][1]
    ...
    my_colors = ['#eb3c0c', '#ff4917', '#EF553B', '#faa328', '#fad028', '#fae528', '#fcef35', '#faf328', '#effa28', '#dbfa28', '#cdfa28', '#adfa28', '#85f714', '#10e63b', '#09d932', '#08cf2f', '#07b329', '#069e24', '#048a1e', '#04801c', '#036917']
    my_Speed = ['1.0 m/s', '1.1 m/s', '1.2 m/s','1.3 m/s','1.4 m/s','1.5 m/s','1.6 m/s','1.7 m/s','1.8 m/s','1.9 m/s', '2.0 m/s', '2.1 m/s', '2.2 m/s', '2.3 m/s', '2.4 m/s', '2.5 m/s', '2.6 m/s', '2.7 m/s', '2.8 m/s', '2.9 m/s',  '3.0 m/s']
...
# https://towardsdatascience.com/multiple-tabs-are-now-part-of-streamlit-9f15169aab9a
...
#csvs = pd.read_csv('https://raw.githubusercontent.com/sandroormeno/app_for_running/main/data/data.csv')
...
#    lista_fechas.append(str(csvs.iloc[i]["name"]))
    
#tabs = st.tabs([lista_fechas[i] for i in range(len(lista_fechas))])

___ = '''
tab1, tab2 = st.tabs(["31 de marzo", "29 de marzo"])

with tab1:
    map_sby = folium.Map(location=[-12.1070348,-76.9468047], zoom_start=13, tiles='cartodbpositron')
    data = pd.read_csv(f'https://raw.githubusercontent.com/sandroormeno/app_for_running/main/data/GPS-3-31-2023.CSV') # GPS.CSV
    ini = csvs.iloc[-1]["ini"]
    mid = csvs.iloc[-1]["mid"]
    fin = csvs.iloc[-1]["fin"]
    st.write(len(data), ini, mid, fin)
    show_maps(view_data, (ini, fin), data)


with tab2:
    map_sby = folium.Map(location=[-12.1070348,-76.9468047], zoom_start=13, tiles='cartodbpositron')
    data = pd.read_csv(f'https://raw.githubusercontent.com/sandroormeno/app_for_running/main/data/GPS-3-29-2023.CSV') # GPS.CSV
    ini = csvs.iloc[-2]["ini"]
    mid = csvs.iloc[-2]["mid"]
    fin = csvs.iloc[-2]["fin"]
    st.write(len(data), ini, mid, fin)
